...
class TransactionPay extends StatefulWidget {
  ...
  TransactionPay({Key? key, required this.isGaveSelected, required this.name,required this.docid}) : super(key: key);
...
class _TransactionPayState extends State<TransactionPay> {
  ...
  void addToGaveCollection( String amount) {
    ...
    customers.doc('${widget.docid}').collection('youGave').add({
      'amount': amount,
      'timestamp': DateTime.now(), // Optional: You can add a timestamp for the transaction
    }).then((value) {
